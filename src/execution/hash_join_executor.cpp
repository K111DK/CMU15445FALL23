...
void HashJoinExecutor::Init() {
  ...
  while (true) {
    ...
    JoinKey key = MakeJoinKey(&tp, left_child_->GetOutputSchema(), plan_->left_key_expressions_);
    JoinValue val = MakeJoinValue(&tp, left_child_->GetOutputSchema());
    ht_.Insert(key, val);
  }
}
...
POP_BACKUP:
  ...
  while (true) {
    ...
    JoinKey key = MakeJoinKey(&tp_temp, right_child_->GetOutputSchema(), plan_->right_key_expressions_);
    JoinValue val = MakeJoinValue(&tp_temp, right_child_->GetOutputSchema());
    ...
    for (auto &bucket_val : join_bucket_ptr->val_bucket_) {
      ...
      ret_val.insert(ret_val.end(), bucket_val.val_.begin(), bucket_val.val_.end());
      ret_val.insert(ret_val.end(), val.val_.begin(), val.val_.end());
  ...
  if (!join_side_scan_ && plan_->GetJoinType() == JoinType::LEFT) {
    ...
    while (iter != ht_.End()) {
      for (auto &join_val : iter.Val().val_bucket_) {
        ...
        std::vector<Value> ret_val{};
        ret_val.insert(ret_val.end(), join_val.val_.begin(), join_val.val_.end());
        ret_val.insert(ret_val.end(), null_right_val.begin(), null_right_val.end());
        backup_tuple_.emplace_back(ret_val, &plan_->OutputSchema());
      }
      ++iter;
    }
