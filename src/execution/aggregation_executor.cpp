...
namespace bustub {
// explain select office_hour, sum(office_hour), max(office_hour), count(*), count(office_hour=1) from
// __mock_table_tas_2023_fall group by office_hour;
// Agg   {
...
auto AggregationExecutor::Next(Tuple *tuple, RID *rid) -> bool {
  while (!done_) {
    ...
    while (aht_iterator_ != aht_.End()) {
      output_val.insert(output_val.end(), aht_iterator_.Key().group_bys_.begin(), aht_iterator_.Key().group_bys_.end());
      output_val.insert(output_val.end(), aht_iterator_.Val().aggregates_.begin(),
                        aht_iterator_.Val().aggregates_.end());
      *tuple = {std::move(output_val), &plan_->OutputSchema()};
      ++aht_iterator_;
      return true;
    }
    done_ = true;
    if (agg_cnt_ == 0 && plan_->group_bys_.empty()) {
      ...
      output_val.insert(output_val.end(), agg_key.group_bys_.begin(), agg_key.group_bys_.end());

      output_val.insert(output_val.end(), agg_val.aggregates_.begin(), agg_val.aggregates_.end());
