  ...
  while (!done_) {
    ...
    if (!right_status) {
      ...
      if (plan_->GetJoinType() == JoinType::LEFT && !right_found_) {
        std::vector<Value> left_join_val = GetValueVectorFromTuple(&left_executor_->GetOutputSchema(), &left_tuple_);
    ...
    auto join_result = plan_->predicate_->EvaluateJoin(&left_tuple_, left_executor_->GetOutputSchema(), &right_tuple_,
    ...
    if (join_result.CompareEquals(true_val) == CmpBool::CmpTrue) {
      std::vector<Value> left_join_val = GetValueVectorFromTuple(&left_executor_->GetOutputSchema(), &left_tuple_);
      std::vector<Value> right_join_val = GetValueVectorFromTuple(&right_executor_->GetOutputSchema(), &right_tuple_);
      left_join_val.insert(left_join_val.end(), right_join_val.begin(), right_join_val.end());
      *tuple = {left_join_val, &plan_->OutputSchema()};
      right_found_ = true;
      return true;
