...
auto UpdateExecutor::Next([[maybe_unused]] Tuple *tuple, RID *rid) -> bool {
  ...
  while (!delete_done_) {
    ...
    if(!is_same_transaction) {
      auto [modified_tp, modified_fields] = GetTupleModifyFields(&child_executor_->GetOutputSchema(),
                                                                 &child_tuple,
                                                                 &insert_tuple);
      ...
      auto original_tp = ReconstructTuple(&child_executor_->GetOutputSchema(),
                                          tp,
                                          meta,undo_logs);
      auto [modified_tp, modified_fields] = GetTupleModifyFields(&child_executor_->GetOutputSchema(),
                                                                 &original_tp.value(),
